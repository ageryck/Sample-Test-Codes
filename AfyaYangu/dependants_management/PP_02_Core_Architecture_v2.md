...
graph TB
    ...
    style AYBFF fill:#e74c3c,color:#fff
...
type Query {
  ...
  availableSchemes: [InsuranceScheme!]!
}
...
type Mutation {
  ...
  addSchemeBeneficiary(enrollmentId: ID!, dependentId: ID!): Beneficiary!
...
type Subscription {
  ...
  householdMemberAdded: HouseholdMember!
}
...
const resolvers = {
  Query: {
    ...
    myHousehold: async (_, __, context) => {
      const patientId = context.user.patientId;
      
      // Fetch household members from CR
      const household = await householdService.getHousehold(patientId);
      return household;
    },
    ...
    availableSchemes: async (_, __, context) => {
      const patientId = context.user.patientId;
      
      // Get household size for eligibility
      const household = await householdService.getHousehold(patientId);
      
      // Fetch available schemes from BMS
      const schemes = await bmsClient.getPublicSchemes();
      
      // Annotate with eligibility
      return schemes.map(scheme => ({
        ...scheme,
        eligibleForHousehold: household.totalSize <= scheme.maxMembers,
        estimatedPremium: calculatePremium(scheme, household.totalSize)
      }));
    }
  },
  
  Patient: {
    household: async (patient, _, context) => {
      return context.loaders.householdLoader.load(patient.id);
    },
  ...
  Household: {
    insuranceEnrollments: async (household, _, context) => {
      // Get all enrollments where household members are primary or beneficiaries
      const memberIds = household.members.map(m => m.patient.id);
      return bmsClient.getHouseholdEnrollments(memberIds);
    }
  },
  
  Mutation: {
    addHouseholdDependent: async (_, { input }, context) => {
      const { nationalId, relationship, demographics } = input;
      const primaryMemberId = context.user.patientId;
      
      // Validate one-household rule
      const validation = await householdService.validateDependentEligibility(
        input.patientId || nationalId
      );
      ...
      // Add dependent
      const member = await householdService.addDependent({
        primaryMemberId,
        ...input
      });
    ...
    addSchemeBeneficiary: async (_, { enrollmentId, dependentId }, context) => {
      ...
      // Verify dependent is in household
      const isInHousehold = await householdService.isHouseholdMember(
        context.user.patientId,
        dependentId
      );
      
      if (!isInHousehold) {
        throw new Error('Can only add household dependents as beneficiaries');
      }
  ...
  Subscription: {
    ...
    householdMemberAdded: {
      subscribe: (_, __, context) => {
        const patientId = context.user.patientId;
        return pubsub.asyncIterator(`household-${patientId}`);
      }
    }
  }
};
...
- Validate one-household-per-dependent rule
- Handle household transfers

#### Insurance Enrollment Service
**Responsibilities**:
- Fetch insurance enrollments from BMS
- Query available schemes
- Add/remove beneficiaries from household
...
CREATE TABLE households (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    primary_member_id VARCHAR(100) NOT NULL,
...
CREATE TABLE household_members (
    ...
    status VARCHAR(20) DEFAULT 'ACTIVE',
    UNIQUE(patient_id, status) WHERE status = 'ACTIVE'  -- Enforce one active household
);
...
CREATE INDEX idx_household_primary ON households(primary_member_id);
CREATE INDEX idx_household_member_patient ON household_members(patient_id);
CREATE INDEX idx_household_member_household ON household_members(household_id);
...
# Household structure (30 minutes TTL)
household:{patientId}
...
- `household_history` - Household membership changes
...
# RelatedPerson operations (Household)
POST   /fhir/RelatedPerson              # Create relationship
GET    /fhir/RelatedPerson?patient={id} # Get household members
PUT    /fhir/RelatedPerson/{id}         # Update relationship
...
- `beneficiary.added` - New beneficiary added to scheme
- `household.member_added` - New household member
...
**Consumer Groups**:
- `afya-yangu-benefits` - Benefit update consumers
- `afya-yangu-clinical` - Clinical data consumers
- `afya-yangu-household` - Household update consumers
- `afya-yangu-notifications` - Notification triggers
...
graph TB
    ...
    subgraph "Application Layer - Kubernetes"
        ...
        AY1[Afya Yangu Pod 1]
        AY2[Afya Yangu Pod 2]
        AY3[Afya Yangu Pod 3]
    ...
    style AY1 fill:#2ecc71,color:#fff
    style BFF1 fill:#e74c3c,color:#fff
    style IOL fill:#9b59b6,color:#fff
```

**Scaling Configuration**:
- **Afya Yangu Pods**: Auto-scale 3-20 pods based on CPU (70% threshold)
...
| Add Household Dependent | <2s | <3s |
| Enroll Beneficiary | <2s | <4s |
| Query Available Schemes | <1s | <2s |
...
```json
{
  ...
  "level": "info",
  "service": "afya-yangu-bff",
  "userId": "patient-123",
  "action": "add_household_dependent",
  "dependentId": "patient-456",
  "relationship": "CHILD",
  "duration_ms": 245,
  "cache_hit": false,
  "status": "success"
