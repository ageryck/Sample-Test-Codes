# Afya Yangu - Registration & Identity Management

## Overview

Patient registration is the entry point to the Afya Yangu, establishing the patient's identity in the Client Registry (CR) and creating login credentials for portal access.
...
sequenceDiagram
    participant User
    participant Portal as Afya Yangu
...

{
  ...
  "identifier": [
    ...
    {
      "use": "secondary",
      "system": "https://afya-yangu.hie.example.com/patient-id",
  ...
  "managingOrganization": {
    ...
    "display": "Afya Yangu Registration"
  },
  "meta": {
    "tag": [
      {
...
async function sendVerificationEmail(user: UserAccount): Promise<void> {
  ...
  const verificationUrl = `https://afya-yangu.hie.example.com/verify-email?token=${token}`;
  
  await emailService.send({
    to: user.email,
    subject: 'Verify Your Afya Yangu Account',
...
async function sendVerificationSMS(user: UserAccount): Promise<void> {
  ...
  await smsService.send({
    to: user.phone,
    message: `Your Afya Yangu verification code is: ${code}. Valid for 10 minutes.`
  });
}
```
